...
// ============================================================
// SUPABASE CONFIG — same bucket used for both modes
// ============================================================
const SUPABASE_URL = import.meta.env.VITE_SUPABASE_URL;
const SUPABASE_ANON_KEY = import.meta.env.VITE_SUPABASE_ANON_KEY;

// ============================================================
// DATABASE LAYER  (Supabase REST API → PostgreSQL table: questions)
// ============================================================
const DB = {
  _h(extra = {}) {
    return {
...
const DB = {
  ...
  async listAll(subject) {
    ...
    const res = await fetch(base, { headers: this._h() });
    const data = await res.json();

    const unique = [...new Set(
      data.map(r => r.file_name?.trim()).filter(Boolean)
    )];

    return unique.map(name => ({ name, size: 0 }));
  },

  // ✅ THIS SHOULD BE OUTSIDE listAll
  async getSubjects() {
...
      rows.push({
        file_name: fileName,
        subject: subject || "general",   // Passed correctly now
        question: cols[0],
        option_a: cols[1] || "",
        option_b: cols[2] || "",
        option_c: cols[3] || "",
...
// Fixed undefined variables inside this function
...
    if (!question || question.toLowerCase() === "question") continue;
    
    const rawOptions = [cols[1], cols[2], cols[3], cols[4]].filter(Boolean);
    const correct = cols[5];
    const solution = cols[6] || "";

    if (!correct || rawOptions.length < 2) continue;
    
    const options = shuffle(rawOptions);
    questions.push({ id: `${i}_${question.slice(0,12)}`, question, options, correct, solution });
  }
  return questions;
}
...
.flip-front {
  background: var(--bg4);
  box-shadow: 0 4px 32px rgba(28,23,16,0.08);
}
.flip-back {
...
.flip-definition {
  font-family: 'DM Sans', sans-serif;
  font-size: 1.08rem; line-height: 1.65; color: var(--ink2); font-weight: 400;
}
.flip-example {
  margin-top: 0.85rem;
  font-family: 'Playfair Display', serif; font-style: italic;
  font-size: 0.88rem; color: var(--muted);
  border-top: 1px solid var(--teal-bdr); padding-top: 0.75rem;
  line-height: 1.55;
}
...
          ))}
          <button
            className={`batch-all-btn ${selected === "all" ? "selected" : ""}`}
...
        <div className="modal-actions">
          <button className="btn btn-teal" style={{flex:1}} onClick={handleStart} disabled={selected === null}>
            Start Flashcards →
          </button>
          <button className="btn btn-ghost" onClick={onClose}>Cancel</button>
        </div>
...
            {card.example && (
              <div className="flip-example">"{card.example}"</div>
            )}
          </div>
        </div>
      </div>
...
  const { questions: initialQ, fileName, title } = session;
...
              setRetries(new Set()); setDeletedCount(0);
            }}>Retry ↺</button>
            <button className="btn btn-ghost" onClick={onExit}>← Back to Sets</button>
          </div>
        </div>
...
      setDone(true);
    } else {
      const newQi = Math.min(qi, newQueue.length - 1);
      setQi(newQi);
      setSelected(null);
      setAnswered(false);
    }
...
    setSaving(true);
    try {
      await DB.deleteQuestion(idToRemove);
    } catch (err) {
...
            Error Detection
            {deletedCount > 0 && (
              <span className="deleted-badge" style={{marginLeft:"0.75rem"}}>
...
// ============================================================
// DASHBOARD — shared for both modes
// ============================================================
  ...
  async function loadSubjects() {
    try {
      ...
      setSelectedSubject(prev =>
        finalSubs.includes(prev) ? prev : finalSubs[0]
      );
...
const addSubject = async () => {
  ...
  try {
    // insert dummy row so subject exists in DB
    await fetch(`${SUPABASE_URL}/rest/v1/gkgs`, {
      method: "POST",
      headers: DB._h(),
      body: JSON.stringify([{
...
 const addSubject = async () => {
  ...
  try {
    // insert dummy row just to register subject
    await fetch(`${SUPABASE_URL}/rest/v1/gkgs`, {
      method: "POST",
      headers: DB._h(),
      body: JSON.stringify([{
...
  // Passes selectedSubject to listAll
...
    setUploading(true);
    try {
      // Pass the selected subject along with the file
...
    if (type === "flash") {
      const data     = cards;
      const selected = batch === "all"
        ? shuffle([...data])
        : shuffle(data.slice(batch.start, batch.end + 1));
      onStartFlash({ cards: selected, title });
    } else {
      const data     = questions;
      const selected = batch === "all"
        ? shuffle([...data])
        : shuffle(data.slice(batch.start, batch.end + 1));
      onStartTest({ questions: selected, fileName, title });
    }
...
        <button className="btn btn-red" onClick={addSubject}>
          Add
        </button>
      </div>
...
                <button
                  className={`btn ${isFlash ? "btn-teal" : "btn-red"} btn-sm`}
...
      {batchPicker && (
        <BatchSelectorModal
          totalCards={batchPicker.type === "quiz"
            ? batchPicker.questions.length
            : batchPicker.cards.length}
...
            </button>
            <button
              className={`mode-tab flash ${mode === "flash" ? "active" : ""}`}
...
          <Dashboard
            mode={mode}
            onStartTest={setQuizSession}
            onStartFlash={setFlashSession}
            toast={toast}
          />
        )}
      </div>
...
when i try to add a new subject , it showing in ui and once i refresh its gone
